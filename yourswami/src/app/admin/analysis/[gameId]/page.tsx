...
export default function GameAnalysisPage({ params }: { params: Promise<{ gameId: string }> }) {
    ...
    const loadGameData = async () => {
        ...
        try {
            ...
            const gamesRes = await adminFetch(`/api/admin/games`);
            ...
            if (!g) {
                ...
                for (let offset = -1; offset >= -3; offset--) {
                    ...
                    const res = await adminFetch(`/api/admin/games?date=${dateStr}`);
            ...
            const [homeRes, awayRes, oddsRes] = await Promise.all([
                adminFetch(`/api/admin/teams/${g.teams.home.id}/stats?season=${season}&league=${leagueId}`),
                adminFetch(`/api/admin/teams/${g.teams.away.id}/stats?season=${season}&league=${leagueId}`),
                adminFetch(`/api/admin/odds/${g.id}`),
            ]);
            ...
            if (hPitcher?.id) {
                const pRes = await adminFetch(`/api/admin/pitchers/${hPitcher.id}?gameType=${gameType}`);
                setHomeP(await pRes.json());
            }
            if (aPitcher?.id) {
                const pRes = await adminFetch(`/api/admin/pitchers/${aPitcher.id}?gameType=${gameType}`);
...
function AITab({ game, homeData, awayData, homeP, awayP, oddsData }: {
    ...
    const runEngine = async (engine: 'stats' | 'pattern') => {
        ...
        setError('');
        try {
            const res = await adminFetch('/api/admin/ai/analyze', {
                method: 'POST',
    ...
    const buildFreebieContent = (rawAnalysis: string): string => {
        ...
        return [`🎯 FREE PICK DROP 🎯`, `━━━━━━━━━━━━━━━━━━━━━`, ``, `⚾ ${matchup}`, `📅 ${gameDate}`, ``, cleanedAnalysis, ``, `━━━━━━━━━━━━━━━━━━━━━`, `💎 Powered by YourSwami Algorithm`, `🔥 Want MORE picks daily? Upgrade at yourswami.com/pricing`].join('\n');
    };
