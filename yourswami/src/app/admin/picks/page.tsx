...
export default function PicksPage() {
    ...
    const fetchPicks = async () => {
        ...
        try {
            ...
            const res = await adminFetch(`/api/admin/picks?from=${today}&limit=50`);
    ...
    const autoGenerateReason = async () => {
        ...
        setGenerating(true);
        try {
            const res = await adminFetch('/api/admin/ai/reason', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
    ...
    const deletePick = async (id: string) => {
        try {
            await adminFetch(`/api/admin/picks/${id}`, { method: 'DELETE' });
    ...
    const updateResult = async (id: string, result: string) => {
        try {
            const res = await adminFetch(`/api/admin/picks/${id}`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
